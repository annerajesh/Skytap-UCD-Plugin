...
skytapClient.defaultRequestHeaders.'Authorization: Basic' = encodedAuthString
...
if (vmName.isInteger()) {
	...
	while ((loopCounter <= 12) && (locked == 1)) {
		try {
			...
			locked = 0
			response = skytapClient.get(path: "templates/" + templateID + "/vms/" + vmID)
...
while ((loopCounter <= 12) && (locked == 1)) {
	try {
		loopCounter = loopCounter + 1
		locked = 0
		response = skytapClient.get(path: "templates/" + templateID)
	} catch(HttpResponseException ex) {
		if ((ex.statusCode == 423) || (ex.statusCode == 500)) {
			println "Looking for specified VM but Environment is locked or busy. Retrying..."
			locked = 1
...
while ((loopCounter <= 12) && (locked == 1)) {
	try {
		loopCounter = loopCounter + 1
		locked = 0
		response = skytapClient.put(path: "configurations/" + configID,
			body: [
...
if (newVMName) {
	...
	while ((success == 0) && (loopCounter <=10)) {
		...
		loopCounter = loopCounter + 1
		try {
			response = skytapClient.put(path: "configurations/" + configID + "/vms/" + newVMID, query: [name:newVMName])
			success = 1
		} catch (HttpResponseException ex) {
...
apTool.setOutputProperty("vmID", newVMID)
