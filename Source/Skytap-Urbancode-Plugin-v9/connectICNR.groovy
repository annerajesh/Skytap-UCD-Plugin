...
busy = 0
response = skytapRESTClient.get(path: "configurations/" + sourceConfigID)
...
response = skytapRESTClient.get(path: "configurations/" + targetConfigID)
...
if (busy == 0) {
	...
	try {
		response = skytapRESTClient.post(path: "tunnels/", query:[source_network_id:sourceNetID, target_network_id:destNetID])
