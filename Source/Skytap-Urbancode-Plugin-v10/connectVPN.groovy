...
vpnAttached = 1
try {
	response = skytapRESTClient.get(path: "configurations/" + configID + "/networks/" + netID + "/vpns/" + vpnID)
} catch(HttpResponseException ex) {
...
if (vpnAttached == 1) {
	...
	while ((loopCounter <= 12) && (locked == 1)) {
		try {
			...
			locked = 0
			response = skytapRESTClient.post(path: "configurations/" + configID + "/networks/" + netID + "/vpns",
				body: ['vpn_id':vpnID],
...
if (vpnConnected == 1) {
	...
	while ((loopCounter <= 12) && (locked == 1)) {
		try {
			loopCounter = loopCounter + 1
			locked = 0
			response = skytapRESTClient.put(path: "configurations/" + configID + "/networks/" + netID + "/vpns/" + vpnID,
				body: ['connected':"true"],
				requestContentType: ContentType.JSON)
		} catch(HttpResponseException ex) {
			if ((ex.statusCode == 423) || (ex.statusCode == 500)) {
